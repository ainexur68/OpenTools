...
- **Base64 编码解码**（`/tools/base64-converter`）  
  - 输入任意文本，点击「编码为 Base64」立即得到字符串，复制按钮可一键写入剪贴板。  
  - 粘贴 Base64 字符串后点击「解码为文本」，若输入非法会显示错误提示，避免错误数据传播。
- **URL 编码解码**（`/tools/url-encoder`）  
  - 兼容 `encodeURIComponent` / `encodeURI`，可选将空格替换为 `+`，方便拼接查询字符串或表单。  
  - 解码面板支持将 `+` 视为空格并展示失败原因，编码与解码结果均可快速复制。
